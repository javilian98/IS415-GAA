...
You can examine the content of `mpsz` by using the variable below.
...
> **Answer:** That is the default number of elements displayed. To display more than 10, you have to use **print(mpsz, n = x)** where **x** is the desired number of elements you want to render.
...
Next, create a new folder rds inside data folder, then write a new file into rds folder.
...
In the following sub-section, we will delve into a few tmap functions that are used to plot these elements.
...
> **My summary:** By comparing a few of the styles (quantile, equal, jenks/natural breaks, standard deviation), quantile has the highest dependency concentration distributed whereas standard deviation has the lowest, in fact, dependency with negative values.
...
> **My observation:** the higher the number of classes, the more variants and higher precision the plot has, and vice-versa.
...
With reference to the results above, we set break point at 0.60, 0.70, 0.80, and 0.90. In addition, we also need to include a minimum and maximum, which we set at 0 and 100. Our breaks vector is thus c(0, 0.60, 0.70, 0.80, 0.90, 19.00)
